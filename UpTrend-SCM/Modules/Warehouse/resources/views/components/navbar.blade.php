<nav class="navbar">
    ...
    <div class="flex space-x-8 mx-auto w-full justify-center">
        ...
        <a href="/orders/create" class="nav-link">Order</a>
            ...
            @if(\Modules\Warehouse\App\Models\Product::whereColumn('quantity', '<', 'reorder_level')->count() > 0)
                <span class="notification-badge">{{ \Modules\Warehouse\App\Models\Product::whereColumn('quantity', '<', 'reorder_level')->count() }}</span>
            @endif
        </a>
    ...
    <div class="flex items-center space-x-4">
        ...
        <a href="{{ route('warehouse.chat.index') }}"
   class="nav-link {{ request()->routeIs('warehouse.chat.index') ? 'active' : '' }}" title="Chat"
   style="color: rgb(255, 255, 255); width: 30px; height: 30px; display: block;"> <svg class="nav-link-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
