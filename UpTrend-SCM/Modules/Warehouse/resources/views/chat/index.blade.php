...
@section('styles')
    ...
    <style>
        ...
        .user-card:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(0,0,0,0.15); border-color: #059669; }
        .user-avatar { width: 50px; height: 50px; background: linear-gradient(135deg, #059669, #047857); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 1.2rem; }
        ...
        .search-box:focus { outline: none; border-color: #059669; box-shadow: 0 0 0 3px rgba(5,150,105,0.1); }
    </style>
@endsection
        ...
        <div class="max-w-4xl mx-auto">
            <div class="chat-header mb-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold mb-2">Warehouse Messages</h1>
                        <p class="text-green-100">Communicate with team members and suppliers</p>
                    </div>
            ...
            @foreach($groups as $role => $users)
                @if($users->count())
                    ...
                    <h3 class="text-lg font-semibold mt-6 mb-2 text-green-700">{{ $role }}</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                        @foreach($users as $userItem)
                            <a href="{{ route('warehouse.chat.with', $userItem->id) }}" class="block">
