...
use Modules\Warehouse\App\Models\ProductStockDetail;
...
class WarehouseController extends Controller
{
    ...
    {
        return view('warehouse::warehouse.dashboard');
    }

    public function inventory()
    {
        $products = Product::with('stockDetails')->get();
        return view('warehouse::warehouse.inventory', compact('products'));
    }

    public function showAddStock($productId)
    {
        $product = Product::findOrFail($productId);
        return view('warehouse::warehouse.stock.add', compact('product'));
    }
    ...
    public function addStock(Request $request, $productId)
    {
        ...
        if ($request->has('color_quantities')) {
            foreach ($request->color_quantities as $color => $quantity) {
                if ($quantity > 0) {
                    $stockDetail = ProductStockDetail::firstOrCreate(
                        [
    ...
    {
        $product = Product::findOrFail($productId);
        return view('warehouse::warehouse.stock.remove', compact('product'));
    }
    ...
    public function removeStock(Request $request, $productId)
    {
        ...
        if ($request->has('color_quantities')) {
            foreach ($request->color_quantities as $color => $quantity) {
                if ($quantity > 0) {
                    $stockDetail = ProductStockDetail::where([
                        'product_id' => $productId,
                        'color' => $color,
                        'size' => null,
                    ])->first();
    ...
    public function transferStock(Request $request){
        $request->validate([
            'product_id' =>
            'required|
            exists:warehouse_products,product_id',
        ...
        $request->quantity;
        $product->save();
        \Modules\Warehouse\App\Models\Stocktransfer::create([
            'product_id' =>
            $request->product_id,
            'to_branch' =>
            $request->to_branch,
    ...
    public function reports(){
        // Reorder Report
        $reorderProducts = \Modules\Warehouse\App\Models\Product::whereColumn('quantity', '<', 'reorder_level')->get();
        
        // Inventory Summary
        $totalProducts = \Modules\Warehouse\App\Models\Product::count();
        $activeProducts = \Modules\Warehouse\App\Models\Product::where('status', 'Active')->count();
        $inactiveProducts = \Modules\Warehouse\App\Models\Product::where('status', 'Inactive')->count();
        $totalStockValue = \Modules\Warehouse\App\Models\Product::sum(DB::raw('quantity * price'));
        $totalStockUnits = \Modules\Warehouse\App\Models\Product::sum('quantity');
        
        // Category Analysis
        $categoryStats = \Modules\Warehouse\App\Models\Product::selectRaw('category, COUNT(*) as count, SUM(quantity) as total_quantity, AVG(price) as avg_price')
            ->groupBy('category')
            ->get();
        
        // Stock Transfer Report
        $recentTransfers = \Modules\Warehouse\App\Models\StockTransfer::with('product')
            ->orderBy('created_at', 'desc')
            ->limit(10)
            ->get();
        
        // Low Stock Products (below 50% of reorder level)
        $criticalStock = \Modules\Warehouse\App\Models\Product::whereRaw('quantity < (reorder_level * 0.5)')->get();
        
        // Top Products by Quantity
        $topProducts = \Modules\Warehouse\App\Models\Product::orderBy('quantity', 'desc')
            ->limit(10)
            ->get();
        
        // Monthly Stock Movement (last 6 months)
        $monthlyTransfers = \Modules\Warehouse\App\Models\StockTransfer::selectRaw('MONTH(created_at) as month, YEAR(created_at) as year, SUM(quantity) as total_transferred')
        ...
        return view('warehouse::warehouse.reports', compact(
            'reorderProducts',
            'totalProducts',
            'activeProducts', 
            'inactiveProducts',
    ...
    {
        $lowStockProducts = \Modules\Warehouse\App\Models\Product::whereColumn('quantity', '<', 'reorder_level')->get();
        $activeProducts = \Modules\Warehouse\App\Models\Product::where('status', 'Active')->count();
        $totalProducts = \Modules\Warehouse\App\Models\Product::count();
        $totalStock = \Modules\Warehouse\App\Models\Product::sum('quantity');
        
        return view('warehouse::warehouse.alerts', compact('lowStockProducts', 'activeProducts', 'totalProducts', 'totalStock'));
    }

    public function showTransferForm()
    {
        $products = \Modules\Warehouse\App\Models\Product::all();
        $branches = ['Kampala', 'Jinja', 'Arua'];
        return view('warehouse::warehouse.transfer', compact('products', 'branches'));
    }
    ...
    public function handleTransfer(Request $request)
    {
        ...
        $product = \Modules\Warehouse\App\Models\Product::find($request->product_id);
        ...
        \Modules\Warehouse\App\Models\StockTransfer::create($transferData);
    ...
    {
        $transfers = \Modules\Warehouse\App\Models\StockTransfer::orderByDesc('created_at')->get();
        return view('warehouse::warehouse.transfers.index', compact('transfers'));
    }

    public function products()
    {
        $products = \Modules\Warehouse\App\Models\Product::all();
        return view('warehouse::warehouse.products', compact('products'));
    }

    public function productsLadies()
    {
        $products = \Modules\Warehouse\App\Models\Product::where('category', 'Ladies')->get();
        return view('warehouse::warehouse.products_ladies', compact('products'));
    }

    public function productsGentlemen()
    {
        $products = \Modules\Warehouse\App\Models\Product::where('category', 'Gentlemen')->get();
        return view('warehouse::warehouse.products_gentlemen', compact('products'));
    }

    public function editProduct($id)
    {
        $product = \Modules\Warehouse\App\Models\Product::findOrFail($id);
        return view('warehouse::warehouse.edit_product', compact('product'));
    }
    ...
    {
        return view('warehouse::warehouse.create_product');
    }
    ...
    public function storeProduct(Request $request)
    {
        ...
        $product = new \Modules\Warehouse\App\Models\Product();
        $product->name = $request->input('name');
        $product->category = $request->input('category');
