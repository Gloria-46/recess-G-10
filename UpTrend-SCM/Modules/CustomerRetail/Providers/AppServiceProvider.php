    ...
    public function boot(): void
    {
        ...
        view()->composer('*', function ($view) {
            if (auth('retailer')->check()) {
                ...
                $products = \Modules\CustomerRetail\App\Models\Product::where('retailer_id', $retailer->id)->get();
        ...
        View::composer('layouts.retailer', function ($view) {
            $pending = 0;
            if (auth('retailer')->check()) {
                $pending = \Modules\CustomerRetail\App\Models\Order::where('retailer_id', auth('retailer')->id())
