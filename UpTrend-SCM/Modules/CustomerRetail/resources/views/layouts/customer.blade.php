...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 min-h-screen bg-pattern">
    <!-- DEBUG AUTH STATUS -->
    {{-- <div style="background: #fffbe6; color: #b45309; padding: 10px; border: 1px solid #fde68a; margin-bottom: 10px; font-size: 15px;">
        <strong>DEBUG:</strong><br>
        Default Auth::check(): {{ Auth::check() ? 'YES' : 'NO' }}<br>
        Customer Auth::guard('web')->check(): {{ Auth::guard('web')->check() ? 'YES' : 'NO' }}<br>
        User: {{ Auth::user() ? Auth::user()->email : 'None' }}<br>
        Customer User: {{ Auth::guard('web')->user() ? Auth::guard('web')->user()->email : 'None' }}
    </div> --}}
    ...
    <nav class="bg-[#23235b] shadow-2xl mb-4 glass-effect sticky top-0 z-50 h-20 backdrop-blur-lg bg-opacity-80 border-b border-blue-200/30 transition-all duration-300" aria-label="Main Navigation">
        <div class="container mx-auto px-6 py-4 flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-x-4 relative h-12 flex-shrink-0" x-data="{ openProfileDropdown: false }">
                @auth
                    <a href="{{ route('customer.chat.index') }}" class="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 hover:scale-105 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 mr-2" title="Chat" aria-label="Chat">
                        <i class="fas fa-comments text-white text-lg"></i>
                    </a>
                @endauth
                @guest
                    <div class="flex items-center h-12 gap-3 whitespace-nowrap">
                        <a href="{{ route('register') }}" class="flex items-center gap-2 text-white hover:text-yellow-200 font-medium transition h-full whitespace-nowrap px-3 py-2 rounded-xl hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400" aria-label="Sign Up"><i class="fas fa-user-plus"></i> Sign Up</a>
                        <a href="{{ route('login') }}" class="flex items-center gap-2 text-white hover:text-yellow-200 font-medium transition h-full whitespace-nowrap px-3 py-2 rounded-xl hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400" aria-label="Login"><i class="fas fa-sign-in-alt"></i> Login</a>
                    </div>
                @else
                    <div class="flex items-center h-12 gap-3 whitespace-nowrap">
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="inline h-full flex items-center">
                            @csrf
