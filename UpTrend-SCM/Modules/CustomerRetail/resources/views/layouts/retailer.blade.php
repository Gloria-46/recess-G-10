...
<head>
    ...
    <title>@yield('title', 'Retailer Dashboard')</title>
    ...
    <div class="min-h-screen">
                ...
                <div class="flex items-center justify-between h-16">
                    <div class="flex items-center h-full">
                        ...
                        <div class="hidden md:flex h-full ml-12 space-x-1 relative">
                            @php
                                $retailerNavLinks = [
                                    ['route' => 'retailer.dashboard', 'icon' => 'fa-chart-line', 'label' => 'DASHBOARD'],
                                    ['route' => 'retailer.products.index', 'icon' => 'fa-box', 'label' => 'PRODUCTS', 'extra' => ['retailer.products.*']],
                                    ['route' => 'retailer.orders.index', 'icon' => 'fa-list', 'label' => 'ORDERS', 'pending' => true],
                                    ['route' => 'retailer.inventory', 'icon' => 'fa-warehouse', 'label' => 'INVENTORY'],
                                    ['route' => 'retailer.analytics', 'icon' => 'fa-chart-bar', 'label' => 'ANALYTICS'],
                                ];
                            @endphp
                            @foreach($retailerNavLinks as $link)
                                @php
                            ...
                            <div class="relative group h-16 flex items-center mr-4">
                                <a href="{{ route('retailer.inventory') }}" class="relative focus:outline-none p-2 rounded-lg hover:bg-white/10 transition-colors">
                    ...
                    <div class="flex items-center space-x-4">
                        @auth('retailer')
                        <a href="{{ route('retailer.chat.index') }}" class="relative focus:outline-none p-2 rounded-lg hover:bg-white/10 transition-colors" title="Chat" aria-label="Chat">
                            <div class="w-8 h-8 bg-blue-600/80 rounded-full flex items-center justify-center">
        ...
        <script>
            ...
            function updatePendingOrdersBadge() {
                fetch('/retailer/orders/pending-count')
                    .then(res => res.json())
                    .then(data => {
