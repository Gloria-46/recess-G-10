        ...
        <div id="inventory-animate" class="opacity-0 translate-y-8 transition-all duration-700">
            ...
            <div class="mb-8">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <a href="{{ route('retailer.inventory.addForm') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300 font-semibold">
                ...
                <div class="mb-8 p-6 bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-red-500 rounded-xl shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        @foreach($low_stock_inventories as $product)
                            <div class="bg-white p-3 rounded-lg border border-red-200 shadow-sm hover:bg-blue-50 transition-colors cursor-pointer block">
                                ...
                                <div class="flex items-center justify-between mt-2">
                                    <a href="{{ route('retailer.inventory.addForm', ['product_id' => $product->id]) }}" class="text-xs text-blue-600 flex items-center hover:underline">
                                        <i class="fas fa-plus mr-1"></i> Add Inventory
                                    </a>
                                    <button class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600 text-xs font-semibold ml-2" onclick="alert('Stock request sent for {{ $product->name }}!')">
                                        <i class="fas fa-truck mr-1"></i> Request for Stock
                                    </button>
                                </div>
                            </div>
                        @endforeach
                            ...
                            <tr class="transition-all duration-300 hover:bg-blue-50 {{ ($batch['running_stock'] ?? 0) <= 5 ? 'bg-red-50 border-l-4 border-red-500' : '' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('retailer.inventory.batch.edit', $batch['id']) }}" class="inline-flex items-center px-3 py-2 text-green-700 bg-green-100 rounded-lg hover:bg-green-200 transition-colors">
                                            <i class="fas fa-edit mr-1"></i>
                                            Edit
                                        </a>
                                        <form action="{{ route('retailer.inventory.batch.delete', $batch['id']) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this batch? This action cannot be undone.');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <a href="{{ route('retailer.inventory.addForm') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                                            <i class="fas fa-plus mr-2"></i>
                                            Add Inventory
                                        </a>
                                    </div>
