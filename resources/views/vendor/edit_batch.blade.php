...
<main class="max-w-2xl mx-auto py-8">
    ...
    <form action="{{ route('vendor.inventory.batch.update', $batch->id) }}" method="POST" class="bg-white shadow rounded-lg p-6 space-y-4">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            ...
            <input type="text" name="supplier_name" id="supplier_name" value="{{ old('supplier_name', $batch->supplier_name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        </div>
        <div>
        ...
        <div class="flex justify-end">
            <a href="{{ route('vendor.inventory') }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 mr-2">Cancel</a>
